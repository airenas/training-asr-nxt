...
use runner::{utils::{duration::format_duration_most_significant, files::{collect_files, run}}, FileParams, APP_NAME};
...
async fn main_int(args: Args) -> anyhow::Result<()> {
    ...
    let files = collect_files(&args.input, &args.extensions)?;
